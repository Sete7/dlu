...
class PostDAO {
    ...
    public function Cadastrar(Post $post) {
        try {
            $sql = "INSERT INTO post (titulo,categoria,url,descricao,valor,status,thumb,data) "
                    . "VALUES (:titulo, :categoria, :url, :descricao, :valor, :status, :thumb, :data)";
            $param = array(
    ...
    public function retornaUrlPost($url) {
        try {
            $sql = "SELECT p.cod, p.titulo, p.categoria, p.url, p.descricao, p.status, p.thumb, p.data,"
                    . " c.cod as codCat, c.titulo as tituloCat FROM post p "
    ...
    //retorna uma lista de pesquisa page para single-search
    public function buscarPost($termo) {
        try {
            
            $sql = "SELECT * FROM post WHERE titulo LIKE :termo ORDER BY cod DESC";            
            ...
            foreach ($dt as $pts):
                ...
                $post->setCod($pts['cod']);
                $post->setTitulo($pts['titulo']);
                $post->setCategoria($pts['categoria']);               
                $post->setStatus($pts['status']);                               
                $post->setUrl($pts['url']);
                $post->setThumb($pts['thumb']);
                $post->setDescricao($pts['descricao']);
